...
export default function ProductList() {
    ...
    return (
        <section className="relative w-full min-h-screen py-32 px-6 bg-white">
            <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
                <div className="absolute top-[10%] left-[20%] w-[40%] h-[30%] blur-[150px] rounded-full opacity-20 bg-sky-200" />
            </div>
                ...
                <motion.div layout className={`grid gap-12 transition-all duration-700 ${getGridLayout()}`}>
                    <AnimatePresence mode='popLayout'>
                        {filteredProducts.map((product, index) => (
                            ...
                            >
                                <div className="relative aspect-square rounded-[2.5rem] border overflow-hidden transition-all duration-700 bg-black/[0.02] border-black/[0.05] group-hover:bg-white group-hover:shadow-2xl group-hover:shadow-blue-500/10">
                                    ...
                                    <div className="absolute top-8 right-8 overflow-hidden">
                                        <div className="w-12 h-12 rounded-2xl flex items-center justify-center transition-all transform translate-x-16 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 bg-black text-white"><HiOutlineArrowUpRight size={22} /></div>
                                    </div>
                                </div>
                                <div className="mt-8 text-center sm:text-left px-4">
                                    <span className="text-[11px] font-black tracking-[0.25em] uppercase transition-colors text-sky-600">{product.category}</span>
                                    <h3 className="text-3xl font-bold mt-2 transition-colors text-slate-900">{product.name}</h3>
                                </div>
                            </motion.div>
                        ))}
                    </AnimatePresence>
