      ...
      {isOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 md:p-6 lg:p-12">
          ...
          <motion.div
            ...
            className="relative w-full max-w-5xl mt-20 h-[80vh] md:h-auto md:max-h-[80vh] overflow-hidden rounded-[2rem] shadow-2xl flex flex-col md:flex-row bg-white"
          >
            {/* Close Button */}
            <button
              onClick={onClose}
              className="absolute top-4 right-4 z-[110] p-2 rounded-full transition-all duration-300 bg-black/5 hover:bg-black/10 text-black"
            >
              <IoClose size={22} />
            </button>

            {/* SOL: Görsel Galerisi (Kompaktlaştırıldı) */}
            <div className="relative w-full md:w-1/2 h-[35vh] md:h-auto flex items-center justify-center p-6 md:p-10 bg-neutral-50">
              ...
              {product.image.length > 1 && (
                <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between pointer-events-none">
                  <button onClick={handlePrev} className="pointer-events-auto p-2 rounded-lg backdrop-blur-md bg-black/5 text-black">
                    <IoChevronBack size={18} />
                  </button>
                  <button onClick={handleNext} className="pointer-events-auto p-2 rounded-lg backdrop-blur-md bg-black/5 text-black">
            ...
            <div className="w-full md:w-1/2 flex flex-col p-6 md:p-10 overflow-y-auto">
              <div className="flex-1">
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  <span className="text-[8px] font-bold tracking-[0.2em] uppercase px-2.5 py-1 rounded-md border text-sky-600 border-sky-600/20 bg-sky-50">
                    {product.category}
                  </span>
                  <h2 className="text-2xl md:text-4xl font-extrabold mt-4 tracking-tight text-neutral-900">
                    {product.name}
                  </h2>
                </motion.div>

                <div className="h-[1px] w-8 my-6 bg-black/5" />

                <p className="text-sm md:text-base font-normal leading-relaxed opacity-80 text-neutral-600">
                  {product.description}
                </p>
              ...
              <motion.div className="mt-8 pt-6 border-t border-white/5">
                <a
                  href="/contact"
                  className="group flex items-center justify-between w-full p-4 rounded-xl font-bold transition-all duration-300 bg-neutral-900 text-white hover:bg-blue-600"
                >
